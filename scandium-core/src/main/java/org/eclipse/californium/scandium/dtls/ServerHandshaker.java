...
public class ServerHandshaker extends Handshaker {
	...
	/** 接收到所有的握手消息的哈希值，这个值将包括在finished消息中进行发送*/
	...
	private void receivedClientHello(final ClientHello clientHello) throws HandshakeException {
		...
		Iterator<String> it = clientHello.getIdentity_list().iterator();

		//对ClientHello消息中的identity_list进行遍历解析，如果有自己需要的identity，就生成psk
		while(it.hasNext()){
			identity = it.next();
			//如果ClientHello中的identity列表中支持二者的identity，就生成预分配密钥
			psk = pskStore.getKey(identity);
			//如果这个identity可以在pskStore中找到对应的密钥，那么就用这个
			if(psk != null) {
		...
		//这次在ServerHello报文中添加identity字段，为了让Client也生成对称密钥
		createServerHello(identity, clientHello, flight);

		/*下面三个报文都不需要再生成，减少了报文数量*/
...
//		createCertificateRequest(clientHello, flight);
		 ...
		 */
		//ServerHelloDone也不需要生成了，直接生成ChangeCipherSpec和Finish报文即可
...
//		handshakeMessages = ByteArrayUtils.concatenate(handshakeMessages, serverHelloDone.toByteArray());
		...
		//放在一次flight中进行发送，这次flight中包括了serverHello消息、ChangeCipherSpec消息以及Finished消息
		recordLayer.sendFlight(flight);
	}
	...
	private void createServerHello(String identity, final ClientHello clientHello, final DTLSFlight flight) throws HandshakeException {
		...
		System.out.println("Client和Server进行加密套件的协商...");
		negotiateCipherSuite(clientHello, serverHelloExtensions);
		System.out.println("Client和Server进行加密套件的协商结束");
		...
		//在ServerHello报文中添加identity字段，通知客户端自己选定的psk
		ServerHello serverHello = new ServerHello(identity, serverVersion, serverRandom, sessionId,
	...
	private void createServerKeyExchange(final ClientHello clientHello, final DTLSFlight flight) throws HandshakeException {
		...
		case PSK:
			...
			System.out.println("客户端的ClientHello采用的是基于PSK握手");
	...
	private void negotiateCipherSuite(final ClientHello clientHello, final HelloExtensions serverHelloExtensions) throws HandshakeException {
		...
		for (CipherSuite cipherSuite : clientHello.getCipherSuites()) {
			...
			System.out.println("当前服务端支持的加密套件有："+supportedCipherSuites);
			System.out.println("当前客户端发来的ClientHello中包括的加密套件是："+cipherSuite);
			if (cipherSuite != CipherSuite.TLS_NULL_WITH_NULL_NULL && supportedCipherSuites.contains(cipherSuite)) {
				if (isEligible(cipherSuite, supportedServerCertType, supportedClientCertType, group)) {
					...
					System.out.println("服务端设置加密套件"+cipherSuite);
