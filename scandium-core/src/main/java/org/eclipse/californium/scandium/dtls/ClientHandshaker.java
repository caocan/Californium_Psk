...
import org.eclipse.californium.scandium.util.Timer;
...
public class ClientHandshaker extends Handshaker {
		...
		case HANDSHAKE:
			...
			case SERVER_KEY_EXCHANGE:
				...
				case EC_DIFFIE_HELLMAN:
					System.out.println("服务端采用的是基于ECC握手");
					receivedServerKeyExchange((ECDHServerKeyExchange) handshakeMsg);
					break;

				case PSK:
					System.out.println("服务端采用的是基于PSK握手");
	...
	/**
	 * 修改后的receivedServerFinished，用来接收服务器发送来的ChangeCipherSpec消息和Finished消息，
	 * 并且根据服务器发来的消息计算哈希值，根据哈希值在客户端发送ChangeCipherSpec消息和Finished消息
	 * @param message
	...
	private void receivedServerFinished(Finished message) throws HandshakeException, GeneralSecurityException {
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "Cannot compute digest for client's Finish handshake message", e);
		}
	...
	protected void receivedHelloVerifyRequest(HelloVerifyRequest message) throws HandshakeException {
		...
		/*向ClientHello的identity_list中添加与服务器之间预分配密钥的identity*/
		//从与对等体对话的session中提取出ip地址以及端口号，利用这个地址到pskStore中寻找对应的identity

		//如果没有，抛出一个没有psk对应identity的异常
		String identity = pskStore.getIdentity(session.getPeer());
		if (identity == null) {
	...
	protected void receivedServerHello(ServerHello message) throws HandshakeException {
        ...
        System.out.println("客户端设置加密套件"+message.getCipherSuite());
        if(message.getCipherSuite() == CipherSuite.TLS_PSK_WITH_AES_128_CCM_8 || message.getCipherSuite() == CipherSuite.TLS_PSK_WITH_AES_128_CBC_SHA256)
			Timer.KeyExchange = "PSK";
	...
	private void receivedServerHelloDone(ServerHelloDone message) throws HandshakeException, GeneralSecurityException {
		...
		//
		String identity = serverHelloDone.getIdentity();
		if (identity == null) {
			AlertMessage alert = new AlertMessage(AlertLevel.FATAL, AlertDescription.HANDSHAKE_FAILURE, session.getPeer());
			throw new HandshakeException("No Identity found for peer: "	+ getPeerAddress(), alert);
		}
		...
		/**
		 * ClientKeyExchange也不需要生成，因为服务器已经协商好了密钥
		 */
		...
		if (certificateRequest != null && negotiatedSignatureAndHashAlgorithm != null) {
			...
			//这句去掉，因为ClientKeyExchange消息已经被删除
...
//		md.update(clientKeyExchange.toByteArray());
	...
	public void startHandshake() throws HandshakeException {
		...
		for (CipherSuite supportedSuite : preferredCipherSuites) {
            System.out.println("ClientHello第一次支持的加密套件是："+supportedSuite);
        ...
        System.out.println("当前clientHello支持的加密套件"+clientHello.getCipherSuites());
